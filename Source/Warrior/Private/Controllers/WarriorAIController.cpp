...
void AWarriorAIController::BeginPlay()
{
	...
	if (UCrowdFollowingComponent* CrowdComp = Cast<UCrowdFollowingComponent>(GetPathFollowingComponent()))
	{
		...
		switch (DetourCrowdAvoidnaceQuality)
		{
		case 1:
...
{
	if (UBlackboardComponent* BlackboarComponent = GetBlackboardComponent())
	{
		if (!BlackboarComponent->GetValueAsObject(FName("TargetActor")))
		{
			if (Stimulus.WasSuccessfullySensed() && Actor)
			{
				BlackboarComponent->SetValueAsObject(FName("TargetActor"), Actor);
			}
		}
	}
}